...
use crate::{DelegatedSafetyConfig, GrevmConfig, ParallelState, Scheduler, TxExecutionOutcome};
use revm::{
    ...
    DB::Error: Send + Sync + Clone + Debug + 'static,
{
    ...
    let actual_bundle = state.finalize_bundle(BundleRetention::Reverts);
    ...
    DB::Error: Send + Sync + Clone + Debug + 'static,
{
    ...
    let parallel_result = (results, state.finalize_bundle(BundleRetention::Reverts));
